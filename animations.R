...
anim <- ggmap(myMap) + 
  geom_point(data=fish1, aes(x=longitude, y=latitude, group = date))+
  transition_time(date)

anim <- ggmap(myMap) + 
  geom_sf(data=fish1, aes(x=longitude, y=latitude, group = date))+
  transition_time(date)
  
gga <- ggplot(data = fish1, aes(x = longitude, y = latitude)) +
